...
extension Datastore {
  ...
  public func put<Entity: _Entity>(
    ...
    let rawEntities: [Google_Datastore_V1_Entity] =
      try entities
      .map { try encoder.encode($0) }

    let result: Google_Datastore_V1_CommitResponse = try await withSpan(
      "datastore-put", ofKind: .client
    ) { span in
  ...
  public func allocateIDs<Key: AnyKey>(
    ...
    let result: Google_Datastore_V1_AllocateIdsResponse = try await withSpan(
      "datastore-allocate-ids", ofKind: .client
    ) { span in
      span.attributes["datastore/kind"] = Key.kind
      return try await withRetryableTask(
        logger: logger,
        operation: { [keys, client] in
          try await client.allocateIds(
            .with {
              $0.projectID = projectID
