...
extension Datastore {
  ...
  public func getEntities<Entity: _Entity>(
    ...
    let response: Google_Datastore_V1_LookupResponse = try await withSpan(
      "datastore-lookup", ofKind: .client
    ) { span in
    ...
    guard
      let updated: Entity =
        (try await getEntities(
          keys: [entity.key], file: file, function: function, line: line))[0]
    else {
