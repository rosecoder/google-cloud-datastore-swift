...
extension Datastore {
  ...
  public func getEntities<Entity: _Entity>(
    ...
    let raws = try await performQuery(
      query: query, projection: [], cursor: &cursor, file: file, function: function, line: line)
  ...
  private func performQuery<Entity: _Entity>(
    ...
    let response: Google_Datastore_V1_RunQueryResponse = try await withSpan(
      "datastore-query", ofKind: .client
    ) { span in
      span.attributes["datastore/kind"] = Entity.Key.kind
      return try await withRetryableTask(
        logger: logger,
        operation: { [client, cursor] in
    ...
    case .moreResultsAfterCursor, .moreResultsAfterLimit, .notFinished:
      if !response.batch.endCursor.isEmpty {
        ...
        if let limit = query.limit {
          cursor =
            response.batch.entityResults.count >= limit
        ...
        logger.error(
          "Datastore query was paginated, but cursor is missing. This may break pagination.")
        cursor = nil
      }
    case .noMoreResults:
      cursor = nil
    case .unspecified:
      if cursor != nil || query.limit != nil {
        logger.error(
          "Datastore query was paginated, but datastore retruned no indication of more data (unspecified batch more results). This may break pagination."
        )
      }
      cursor = nil
    case .UNRECOGNIZED(let rawValue):
      logger.error(
        "Unrecognized datastore query batch more results. This may break pagination.",
        metadata: [
