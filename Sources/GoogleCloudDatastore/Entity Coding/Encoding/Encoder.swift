...
struct EntityEncoder {
    ...
    T.Key: AnyKey
  {
    let encoder = RootEncoder(propertyConfiguration: {
      ($0 as? T.CodingKeys).flatMap { T.propertyConfiguration(key: $0) }
        ?? T.propertyConfiguration(otherKey: $0)
    })
  ...
  final private class RootEncoder: Encoder {
    ...
    where Key: CodingKey {
      ...
      let container = KeyedContainer<Key>(
        codingPath: codingPath, propertyConfiguration: propertyConfiguration)
