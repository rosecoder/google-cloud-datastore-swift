...
@Suite struct EntityCodingTests {
  ...
  @Test func withParentableNamespaceableKey() throws {
    let source = ParentableNamespaceableKeyEntity(
      key: .named("c", parent: .named("p", namespace: .test1), namespace: .test1))
  ...
  private func assert(
    raw: Google_Datastore_V1_Entity, matching entity: AllPropertyTypeEntity,
    ...
    #expect(
      raw.properties["Dictionary"]?.entityValue.properties["a"]?.valueType == .integerValue(1))
    #expect(
      raw.properties["Dictionary"]?.entityValue.properties["b"]?.valueType == .integerValue(2))
    #expect(
      raw.properties["Dictionary"]?.entityValue.properties["c"]?.valueType == .integerValue(3))
  }
}
